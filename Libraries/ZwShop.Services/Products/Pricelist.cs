...
namespace ZwShop.Services.Products
{
    ...
    public partial class Pricelist : BaseEntity
    {
            ...
            while (strHelper.Contains("%"))
            {
                ...
                if (tokens.Get(strToken + strFormat) == null)
                {
                    switch (strToken.ToLower())
                    {
                        ...
                        case "pv.partnumber":
                            {
                                tokens.Add(strToken + strFormat, String.Format(locProvider, "{0" + strFormat + "}", productVariant.BrandPartNumber));
                            }
                            break;
                        case "product.Brand":
                            {
                                string mans = string.Empty;
                                var productBrands = productVariant.Product.ProductBrands;
                                foreach (ProductBrand pm in productBrands)
                                {
                                    mans += ", " + pm.Brand.Name;
                                }
                                if (mans.Length != 0)
        ...
        public static string[] GetListOfAllowedTokens()
        {
            ...
            allowedTokens.Add("%product.Brand%");
