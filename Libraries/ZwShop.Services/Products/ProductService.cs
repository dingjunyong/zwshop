...
using ZwShop.Services.Promo.Discounts;
using ZwShop.Common;
...
namespace ZwShop.Services.Products
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <param name="featuredProducts">A value indicating whether loaded products are marked as featured (relates only to categories and Brands). 0 to load featured products only, 1 to load not featured products only, null to load all products</param>
        /// <param name="pageSize">Page size</param>
        /// <param name="pageIndex">Page index</param>
        /// <param name="totalRecords">Total records</param>
        /// <returns>Product collection</returns>
        ...
        {
            return GetAllProducts(categoryId, BrandId,
        ...
        {
            int languageId = ShopContext.Current.WorkingLanguage.LanguageId;

            return GetAllProducts(categoryId, BrandId, productTagId,
            ...
            ProductSortingEnum orderBy, out int totalRecords)
        {
            return GetAllProducts(categoryId,
                BrandId, productTagId, featuredProducts,
                priceMin, priceMax, 0,
        ...
        /// <param name="BrandId">Brand identifier; 0 to load all recordss</param>
        ...
        {
            if (ShopContext.Current.User == null || (ShopContext.Current.User.IsGuest && !IoC.Resolve<ICustomerService>().AllowAnonymousUsersToSetProductRatings))
            {
                return;
            }
            ...
            var product = GetProductById(productId);
            if (product == null)
                throw new ShopException("Product could not be loaded");
            ...
            var oldPr = (from pr in _context.ProductRatings
                         ...
                         pr.CustomerId == ShopContext.Current.User.CustomerId
            ...
            newPr.CustomerId = ShopContext.Current.User.CustomerId;
            newPr.Rating = rating;
        ...
        {
            HttpCookie compareCookie = HttpContext.Current.Request.Cookies.Get("ShopCommerce.CompareProducts");
        ...
        public void AddProductToCompareList(int productId)
        {
            ...
            HttpCookie compareCookie = HttpContext.Current.Request.Cookies.Get("ShopCommerce.CompareProducts");
            if (compareCookie == null)
                compareCookie = new HttpCookie("ShopCommerce.CompareProducts");
            compareCookie.Values.Clear();
            int maxProducts = 4;
            int i = 1;
            foreach (int newProductId in newProductIds)
        ...
        {
            var productIds = new List<int>();
            HttpCookie recentlyViewedCookie = HttpContext.Current.Request.Cookies.Get("ShopCommerce.RecentlyViewedProducts");
        ...
        public void AddProductToRecentlyViewedList(int productId)
        {
            ...
            if (recentlyViewedCookie == null)
                recentlyViewedCookie = new HttpCookie("ShopCommerce.RecentlyViewedProducts");
        ...
        public List<Product> GetRecentlyAddedProducts(int number)
        {
            ...
            var products = this.GetAllProducts(0,
                ...
                0, null, ShopContext.Current.WorkingLanguage.LanguageId,
            ...
            bool isPublished, bool copyImages)
        {
            ...
            //using (var scope = new System.Transactions.TransactionScope())
            {
                ...
                // product <-> Brands mappings
                foreach (var productBrands in product.ProductBrands)
                {
                    var productBrandCopy = new ProductBrand()
                    {
                        ProductId = productCopy.ProductId,
                        BrandId = productBrands.BrandId,
                        IsFeaturedProduct = productBrands.IsFeaturedProduct,
                        DisplayOrder = productBrands.DisplayOrder
                    };

                    IoC.Resolve<IBrandService>().InsertProductBrand(productBrandCopy);
                }
                ...
                foreach (var productVariant in productVariants)
                {
                    ...
                    var productVariantCopy = new ProductVariant()
                    {
                        ...
                        BrandPartNumber = productVariant.BrandPartNumber,
        ...
        public void InsertProductVariant(ProductVariant productVariant)
        {
            ...
            productVariant.BrandPartNumber = CommonHelper.EnsureNotNull(productVariant.BrandPartNumber);
            productVariant.BrandPartNumber = CommonHelper.EnsureMaximumLength(productVariant.BrandPartNumber, 100);
        ...
        {
            string IPAddress = ShopContext.Current.UserHostAddress;
        ...
        public void SetProductRatingHelpfulness(int productReviewId, bool wasHelpful)
        {
            ...
            if (ShopContext.Current.User.IsGuest && !IoC.Resolve<ICustomerService>().AllowAnonymousUsersToReviewProduct)
            {
                return;
            }
            ...
            var oldPrh = (from prh in _context.ProductReviewHelpfulness
                         ...
                         prh.CustomerId == ShopContext.Current.User.CustomerId
            ...
            newPrh.CustomerId = ShopContext.Current.User.CustomerId;
