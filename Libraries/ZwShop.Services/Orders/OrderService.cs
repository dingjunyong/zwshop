...
namespace ZwShop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
                        ...
                        case (int)GiftCardTypeEnum.Virtual:
                            {
                                    ...
                                    !String.IsNullOrEmpty(gc.SenderEmail))
                                {
                                    ...
                                    if (customerLang == null)
                                        customerLang = ShopContext.Current.WorkingLanguage;
        ...
        {
            return GetOrderNoteByOrderId(orderId, ShopContext.Current.IsAdmin);
        }
            ...
            string message, DateTime createdOn)
        {
            ...
            if (customer == null)
                throw new ShopException("Customer not found. ID=" + customerId);
            ...
            Guid orderGuid, out int orderId)
        {
            ...
            try
            {
                ...
                if (customer.IsGuest && !customerService.AnonymousCheckoutAllowed)
                    throw new ShopException("Anonymous checkout is not allowed");

                if (!CommonHelper.IsValidEmail(customer.Email))
                {
                    throw new ShopException("Email is not valid");
                }
                ...
                if (paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (initialOrder == null)
                        throw new ShopException("Initial order could not be loaded");
                }

                if (!paymentInfo.IsRecurringPayment)
                {
                    if (paymentInfo.BillingAddress == null)
                        throw new ShopException("Billing address not provided");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (warnings.Count > 0)
                    {
                        ...
                        throw new ShopException(warningsSb.ToString());
                    }
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (!String.IsNullOrEmpty(subTotalError1) || !String.IsNullOrEmpty(subTotalError2))
                        throw new ShopException("Sub total couldn't be calculated");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (shoppingCartRequiresShipping)
                    {
                        if (paymentInfo.ShippingAddress == null)
                            throw new ShopException("Shipping address is not provided");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                        throw new ShopException("Shipping total couldn't be calculated");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (!String.IsNullOrEmpty(paymentAdditionalFeeError1))
                        throw new ShopException("Payment method fee couldn't be calculated");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (!String.IsNullOrEmpty(taxError))
                        throw new ShopException("Tax total couldn't be calculated");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (!orderTotal.HasValue)
                        throw new ShopException("Order total couldn't be calculated");
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    if (paymentMethod == null)
                        throw new ShopException("Payment method couldn't be loaded");

                    if (!paymentMethod.IsActive)
                        throw new ShopException("Payment method is not active");
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (billingAddress.Country != null)
                    {
                        ...
                        {
                            throw new ShopException(string.Format("{0} is not allowed for billing", billingCountry));
                        }
                    }
                }
                else
                    ...
                    if (!paymentInfo.IsRecurringPayment)
                    {
                        ...
                        if (shippingAddress != null)
                        {
                            ...
                            if (shippingAddress.Country != null)
                            {
                                ...
                                {
                                    throw new ShopException(string.Format("{0} is not allowed for shipping", shippingCountry));
                                }
                            }
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (isRecurringShoppingCart)
                    {
                        ...
                        {
                            throw new ShopException(recurringCyclesError);
                        }
                ...
                if (!skipPaymentWorkflow)
                {
                        ...
                        if (isRecurringShoppingCart)
                        {
                            ...
                            switch (recurringPaymentType)
                            {
                                case RecurringPaymentTypeEnum.NotSupported:
                                    throw new ShopException("Recurring payments are not supported by selected payment method");
                                ...
                                case RecurringPaymentTypeEnum.Automatic:
                                    ...
                                    break;
                                default:
                                    throw new ShopException("Not supported recurring payment type");
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            throw new ShopException("No recurring products");
                        }
                    }
                }
                else
                ...
                if (String.IsNullOrEmpty(processPaymentResult.Error))
                {
                    ...
                    //using (var scope = new System.Transactions.TransactionScope())
                    {
                        var order = new Order()
                        {
                            ...
                            CustomerIP = ShopContext.Current.UserHostAddress,
        ...
        public void ProcessNextRecurringPayment(int recurringPaymentId)
        {
            try
            {
                ...
                if (rp == null)
                    throw new ShopException("Recurring payment could not be loaded");

                if (!rp.IsActive)
                    throw new ShopException("Recurring payment is not active");
                ...
                if (customer == null)
                    throw new ShopException("Customer could not be loaded");

                var nextPaymentDate = rp.NextPaymentDate;
                if (!nextPaymentDate.HasValue)
                    throw new ShopException("Next payment date could not be calculated");
                ...
                {
                    throw new ShopException(result);
                }
                else
                {
                    var rph = new RecurringPaymentHistory()
        ...
        public Order Ship(int orderId, bool notifyCustomer)
        {
            ...
            if (!CanShip(order))
                throw new ShopException("Can not do shipment for order.");
        ...
        public Order Deliver(int orderId, bool notifyCustomer)
        {
            ...
            if (!CanDeliver(order))
                throw new ShopException("Can not do delivery for order.");
        ...
        public Order CancelOrder(int orderId, bool notifyCustomer)
        {
            ...
            if (!CanCancelOrder(order))
                throw new ShopException("Can not do cancel for order.");
        ...
        public Order Capture(int orderId, ref string error)
        {
            ...
            if (!CanCapture(order))
                throw new ShopException("Can not do capture for order.");
        ...
        public Order MarkOrderAsPaid(int orderId)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new ShopException("You can't mark this order as paid");
        ...
        public Order Refund(int orderId, ref string error)
        {
            ...
            if (!CanRefund(order))
                throw new ShopException("Can not do refund for order.");
        ...
        public Order RefundOffline(int orderId)
        {
            ...
            if (!CanRefundOffline(order))
                throw new ShopException("You can't refund this order");
        ...
        public Order PartiallyRefund(int orderId, decimal amountToRefund, ref string error)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new ShopException("Can not do partial refund for order.");
        ...
        public Order PartiallyRefundOffline(int orderId, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new ShopException("You can't partially refund (offline) this order");
        ...
        public Order Void(int orderId, ref string error)
        {
            ...
            if (!CanVoid(order))
                throw new ShopException("Can not do void for order.");
        ...
        public Order VoidOffline(int orderId)
        {
            ...
            if (!CanVoidOffline(order))
                throw new ShopException("You can't void this order");
